...
5. 完成 Windows 10/11 干净环境 Release Candidate 冒烟。
...
- 构建 RC wheel，在 Windows 10/11 干净环境测试。
...
在干净 Windows 环境安装，按提示完成配置，启动 Desktop，完成一次对话和显式记忆；全过程不查源码、不手动修数据库。

### Case B：有事找朝汐

用户用自然语言开启铁幕、查询当前状态并落幕。朝汐调用正确工具、必要时请求权限、回读事实源，并给出自然语言结果。
...
在空数据目录安装 v1.0，创建 Memory、Plan、Permission、Workflow、Proactive、Reflection 数据；模拟损坏后从 safeguard 恢复。
...
执行混合对话、定时任务、SSE 重连、Voice start/stop 和 Provider 抖动；资源曲线稳定，临时文件和队列有界。
...
- 三条日用闭环的自动化测试和黑盒验收记录。
- Windows 10/11 RC 冒烟记录、soak 报告、构建校验和。
- 已知限制与 P2 后续清单。

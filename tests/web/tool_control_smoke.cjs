...
(async () => {
  ...
  try {
    ...
    await page.locator('#toolCabinetPanel > summary').click();
    const group = page.locator('details[data-group="preview"]');
    await group.locator('summary').click();
    const enabled = page.getByRole('checkbox', {name: 'local_preview 启用', exact: true});
    const forced = page.getByRole('checkbox', {name: 'local_preview 每轮携带', exact: true});
    ...
    assert.equal(result.tools.find(t => t.name === 'local_preview').enabled, true);
    result = await change(() => forced.check());
    assert.equal(result.tools.find(t => t.name === 'local_preview').exposed, true);
    await page.reload();
    await page.locator('#sideToggle').click();
    await page.locator('.maintenance > summary').click();
    await page.locator('#toolCabinetPanel > summary').click();
    ...
    result = await change(() => group.getByRole('button', {name: '停用本组', exact: true}).click());
    assert.equal(result.tools.find(t => t.name === 'local_preview').exposed, false);
    result = await change(() => group.getByRole('button', {name: '恢复本组默认', exact: true}).click());
    assert.equal(result.tools.find(t => t.name === 'local_preview').force_expose, false);
    assert.equal(result.tools.find(t => t.name === 'local_preview').enabled, false);

...
test('live proactive replies use normal reply queue and deduplicate restored deliveries',async()=>{
 const sent=[],notices=[];
 const c=vm.createContext({showProactiveNote:()=>{},seenDeliveries:new Set(['old']),deliveryQueue:[],historyReady:true,busy:true,pendingInput:[],$:()=>({}),showNotice:d=>notices.push(d),sendReply:async(text)=>sent.push(text),finishTurn:()=>{c.busy=false}});
